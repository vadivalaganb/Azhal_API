...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $sql = "SELECT id, icon, header_name, description, status, created_at, updated_at 
            FROM product_content ORDER BY created_at DESC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isUpdate) {
        $stmt = $conn->prepare("UPDATE product_content SET icon=?, header_name=?, description=?, status=? WHERE id=?");
        ...
        $stmt = $conn->prepare("INSERT INTO product_content (icon, header_name, description, status) VALUES (?, ?, ?, ?)");
        if (!$stmt) jsonResponse(['success' => false, 'error' => $conn->error], 500);
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $stmt = $conn->prepare("DELETE FROM product_content WHERE id=?");
    if (!$stmt) jsonResponse(['success' => false, 'error' => $conn->error], 500);
